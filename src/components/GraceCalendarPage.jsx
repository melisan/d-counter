...
const PERSON_STYLE = {
  '미선': { text: 'text-amber-600',  ring: 'focus:ring-amber-300', saveBtn: 'bg-amber-500 hover:bg-amber-600 text-white'  },
  '진욱': { text: 'text-teal-600',   ring: 'focus:ring-teal-300',  saveBtn: 'bg-teal-500 hover:bg-teal-600 text-white'    },
}
...
export default function GraceCalendarPage({ data, adminMode, onEditMemo, onDeleteEntry }) {
  const now = new Date()
  ...
  const monthGrace  = data.graceMonths?.[monthKey] || {}
  ...
  function getDayCount(day, name) {
    return (monthGrace[name]?.[`${year}-${pad(month + 1)}-${pad(day)}`] ?? []).length
  }
                ...
                <div key={ev.ts} className="bg-white rounded-2xl px-4 py-3 shadow-sm flex items-start gap-3">
                  ...
                  {adminMode ? (
                    <div className="flex gap-1 shrink-0">
                      <button onClick={() => startEdit(ev)} className="w-8 h-8 flex items-center justify-center rounded-xl bg-gray-50 hover:bg-gray-100 text-sm transition-colors">✏️</button>
                      <button onClick={() => onDeleteEntry(ev.name, ev.ts)} className="w-8 h-8 flex items-center justify-center rounded-xl bg-gray-50 hover:bg-red-50 text-sm transition-colors">🗑️</button>
                    </div>
                  ) : (
